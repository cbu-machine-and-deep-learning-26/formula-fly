...
12. Integrate central-complex / descending-neuron circuitry between eye and policy using the Shiu et al. LIF model as the base. **Decided (GH-23, Sep 2026): PyTorch in the loop, Brian2 offline, and the in-loop network capped at roughly 1,000-2,000 neurons.** The measurement that matters is cost *per 20 ms frame when stepped one frame at a time*, because that is how a brain between an eye and a policy is driven. Brian2 charges a fixed 33-52 ms of setup on every `run()` call regardless of network size, so stepped it costs 36-59 ms per frame at every size measured -- 4-6x over the ~10 ms budget (the 20 ms frame less the eye's 7.9 ms and the practice track's 2.2 ms). PyTorch charges nothing per call: 5.5 ms at 1,000 neurons, 12.2 ms at 3,000, 77.7 ms at 10,000. Batched into one long run the ranking inverts -- Brian2 is 3.9-7.8 ms across the same sizes and scales with *activity* rather than size -- which is why the lesion sweep (#30) and any offline work stay on Brian2. Do not quote a batched number for an in-loop brain: at 3,000 neurons that is 4.9 ms against a real 54.5 ms. All figures are floors from Brian2's uncompiled numpy target; whether the compiled target reduces the per-call setup is untested and is the one thing that could move this. Full numbers and method in `docs/running-the-stacks.md`.
