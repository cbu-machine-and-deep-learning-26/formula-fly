...
12. Integrate central-complex / descending-neuron circuitry between eye and policy using the Shiu et al. LIF model as the base. **Decided (GH-23, Sep 2026): PyTorch in the loop, Brian2 offline. The in-loop network fits about 10,000 neurons at `dt = 0.5 ms`**, which covers a central complex plus descending neurons (~4,300) with room to spare -- so RQ2 is reachable with a real circuit rather than a toy one. The measurement that matters is cost *per 20 ms frame when stepped one frame at a time*, because that is how a brain between an eye and a policy is driven; a batched number understates Brian2 by an order of magnitude. Brian2 charges a fixed 33-52 ms of setup on every `run()` call regardless of network size, so stepped it is 36-59 ms per frame at every size measured, against a ~10 ms budget (the 20 ms frame less the eye's 7.9 ms and the practice track's 2.2 ms). torch stepped: 4.5 ms at 1,000 neurons, 5.6 at 3,000, 8.7 at 10,000, 16.7 at 20,000. Brian2 keeps the offline work -- the lesion sweep (#30), parameter searches -- where it is 5.8-7.8 ms batched at those sizes, scales with *activity* rather than size, and is the published model. **Two caveats.** All of this is CPU: the CUDA path is written and self-checking but unmeasured, and the lab's 4090s settle it with `--device cpu cuda` on one machine. And these torch figures are ~9x better than the first ones because sparse COO and per-step `.item()` calls were replaced with CSR and a device-resident spike counter -- work done for the GPU that moved the CPU by an order of magnitude, so treat any LIF timing as provisional until the hot loop has been looked at. Full numbers and method in `docs/running-the-stacks.md`.
