...
12. Integrate central-complex / descending-neuron circuitry between eye and policy using the Shiu et al. LIF model as the base. **Decided (GH-23, Sep 2026): Brian2, on a central-complex-scale subnetwork, at `dt = 0.5 ms`.** Measured on the Windows box with Brian2's *uncompiled* numpy target, so these are floors: 3,000 neurons cost 12.9 ms of wall clock per 20 ms of biology and the whole 127,400-neuron brain 101.5 ms, against a budget of ~10 ms (the 20 ms frame less the eye's 7.9 ms and the practice track's 2.2 ms). The whole brain stays offline; the lesion sweep (#30) does not need real time. Brian2 beat a PyTorch LIF not on raw speed but on shape -- Brian2's cost tracks *activity* (17.1 / 23.7 / 27.1 ms at 0% / 1% / 10% of neurons driven) while a sparse-matmul PyTorch version's tracks *size* and is flat (75.7 / 74.4 / 78.0 ms). A fly brain fires at a few Hz, so event-driven propagation does about a thousand times less work, and the gap widens with every neuron added. `dt` is a bigger lever than either backend, but 0.5 ms is the floor: firing rates hold from 0.1 to 0.5 ms and drift +24% by 1.0 ms, where the 1.8 ms synaptic delay and 2.2 ms refractory period stop being resolved. Full numbers, the install, and the sharp edges are in `docs/running-the-stacks.md`.
